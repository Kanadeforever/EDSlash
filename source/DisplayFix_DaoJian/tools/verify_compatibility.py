# -*- coding: utf-8 -*-
"""
DisplayFix layer1d：本体 ComeOn.exe 兼容性只读检查工具。

这个工具只读取 EXE，不写入任何字节，也不会生成补丁版 EXE。兼容判断不依赖整个文件 SHA-256 白名单，
而是检查 DisplayFix 真正依赖的机器码、调用关系、vtable、主 HUD 与辅助 GUI 类结构是否仍然成立。

layer1d 从 v0.3.2 稳定运行基线重新开线。test1~test7 的运行时代码全部废弃，只保留已确认的逆向证据。
因此本工具会比 v0.3.2 旧验证器检查得更深，但“检查某条输入结构存在”绝不等于 layer1d 会改它。

继续验证的稳定基线包括：
- 字体 DPI 原始/已修复状态；
- 两处分辨率映射、JMM 选择器、Strategy enter/exit 与前端 mode 4 生命周期；
- test10 Steam delayed JMM 所需的 0x4087A0 -> 0x4B35F0 与资源路径构造链；
- 世界鼠标按下防穿透与全局 release fallback；
- 主 HUD vtable、0x09~0x0E 六条窗口开关、通用布局、键盘快捷键；
- 0x0B 装备、0x0D 技能、0x0E 物品三个菜单类的 constructor/vtable/+0x08 Draw/+0x24 事件/+0x30 原版命中/+0x58 布局；
- 乾坤袋/物品辅助对象的构造写槽只作为逆向证据保存，layer1d 不移动它。

layer1d 继续依赖的 UI manager / Draw 结构验证包括：
- 唯一 `0x408510` 风格 UI manager Draw 包装结构；
- 包装函数内部真正 call 到 UI manager Draw；
- manager Draw 中“HUD 特殊 pass -> 顶层链 manager+0x1C -> object vtable+0x08 Draw”的稳定结构；
- 主 HUD 与三个辅助菜单的 vtable+0x08 都必须仍指向主 EXE .text。

最重要的边界：本工具会验证 +0x30/self+0xA8/GetCursorPos 等输入证据仍保持原版形状，
正是为了证明 layer1d 仍禁止改这些输入路径；运行时不再改顶层链：绘制继续使用 HUD 后延迟 Draw；输入只覆盖原版 picker 的单次返回值。
"""
...
BAG_AUX_PANEL_VTABLE = 0x0052A60C
BAG_AUX_PANEL_SLOT = 0x0055B784
BAG_AUX_PANEL_CTOR_WRITE = bytes.fromhex("C7 06 0C A6 52 00 89 35 84 B7 55 00")
...
EXPECTED_GET_CURSOR_POS_IAT = 0x005283C8
...
def verify_one(path: Path) -> list[str]:
    ...
    #   3. imm32 必须正好是当前 本体 已确认的 GetCursorPos IAT 0x005283C8。
    #
