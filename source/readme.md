...
以下工具说明主体形成于 layer1d 阶段；stripe1 只在构建验证器中新增 8 像素宽度对齐防回归，其余工具职责保持不变。

DisplayFix 构建验证器负责锁死 Win32/PE32、零导入、中文日志、配置编码、layer1d 不移动 GUI/不写顶层链的边界，并新增检查 `TrackedAuxiliaryObject` 短生命周期机制：只能延续此前在合法菜单上下文中确认的独立面板，而且必须逐帧复核真实 Draw 链、active、vtable 和 HUD 相交。兼容验证器负责真实 ComeOn.exe/ComeOn.dll 的只读结构检查。FontFix 工具仍是独立 96 DPI 离线补丁器，与 layer1d 绘制/输入逻辑没有耦合。

layer1d 明确禁止 test1~test7 的菜单位移/坐标补偿和 layer1b 顶层链写入；输入仍只覆盖原版 picker 的单次返回值，后续 root/focus/child hit-test 继续由原版处理。所有工具的详细参数、验证范围和当前限制见各自目录《工具详细说明.md》。