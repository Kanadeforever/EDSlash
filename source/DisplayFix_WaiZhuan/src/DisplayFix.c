...
static void __fastcall strategy_enter_hook(LPVOID self, LPVOID unused_edx)
{
    ...
    if (force_patch_ok) {
        ...
        if (!force_restore_ok) {
            append_runtime_line("[RUNTIME] Strategy enter force-reapply restore FAILED; Strategy-enter remains force=1");
        }
    }
...
static void initialize_display_fix(void)
{
    ...
    if (resolution_result) {
        ...
        log_line("[INFO] hidden 1024x768 branch selection is not required; gameplay remaps all confirmed mode branches");
    } else {
    ...
    if (strategy_state_result) {
        ...
        log_line("[INFO] enter: original Strategy-enter is forced once; exit: original display mode 4 is forced once after Strategy cleanup");
    ...
    if (GAME_GetModuleHandleA && GAME_GetModuleHandleA("ComeOn.dll")) {
        ...
        if (!jmm_context_result) {
            log_line("[INFO] optional JMM context was not resolved; core non-Steam widescreen path can still continue");
        }
    }
